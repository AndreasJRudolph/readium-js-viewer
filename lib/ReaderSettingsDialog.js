define(['hgn!templates/settings-dialog.html', 'ReaderSettingsDialog_Keyboard', 'i18n/Strings', 'Dialogs', 'storage/Settings', 'Keyboard', 'colpick', 'bootstrap'], function (SettingsDialog, KeyboardSettings, Strings, Dialogs, Settings, Keyboard) {
    ...
    var setUserInterfaceStyles = function () {
        ...
        var CustomUserInterfaceStyles = false;
        Settings.get('reader', function (json) {
            if (json) {
                if (json['theme']) {
                    ...
                    color = rgb2hex(color); //to normalize RGB values e.g. RGB (224, 224, 224) to HEX color values e.g. #e0e0e0
                    backgroundColor = rgb2hex(backgroundColor);

                    CustomUserInterfaceStyles = true;

                }
            }
        });

        if (CustomUserInterfaceStyles) {
            ...
            backgroundColorHover = parseInt(backgroundColorHover, 16) - parseInt("333333", 16);
            if (backgroundColorHover < 0){
                backgroundColorHover += 2*parseInt("333333", 16);
                alert ("-");
            };
            backgroundColorHover = "#" + backgroundColorHover.toString(16);

            alert ("backgroundColorHover: " + backgroundColorHover);
            ...
            for (i = 0; i < numberOfStylesheets; i++) {
                ...
                for (var j = 0; j < rules.length; j++) {
                    if (rules[j].type == 1 && rules[j].selectorText.toLowerCase().indexOf(":hover") > -1) { //find ":hover" rules
                        //rules[j].style.setProperty("color", color, "important");
    ...
    var initCustomThemeDialog = function () {
        ...
        $("#background-color-btn").colpick({
            ...
            onChange: function (hsb, hex, rgb, el, bySetColor) {
                ...
                $('#custom-theme-radio-div').css('background-color', hex);
        ...
        $("#foreground-color-btn").colpick({
            ...
            onChange: function (hsb, hex, rgb, el, bySetColor) {
                ...
                $('#custom-theme-radio-div').css('color', hex);
