define(['hgn!templates/settings-dialog.html', 'ReaderSettingsDialog_Keyboard', 'i18n/Strings', 'Dialogs', 'storage/Settings', 'Keyboard', 'colpick', 'bootstrap'], function(SettingsDialog, KeyboardSettings, Strings, Dialogs, Settings, Keyboard){
    ...
    var setFocusSize = function(size){
        ...
        for (var i=0; i<rules.length; i++){
            if(rules[i].selectorText.toLowerCase().indexOf(":focus")>-1){ //find ":focus" rules
                if (rules[i].style.outlineWidth!="0px"){
                    var alt="alt"+rules[i].cssText;
                    rules[i].style.setProperty("outline-width", size+"px","important");
                    //alert("neu"+rules[i].cssText+"\n"+alt);
                }
                if (rules[i].style.borderWidth!="0px"){
                    rules[i].style.setProperty("border-width", size+"px","important");
                }
            }
        }
    }
